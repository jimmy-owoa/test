class Admin::ProjectTempsController < Admin::ApplicationController
  before_action :set_project_temp, only: [:show, :edit, :update, :destroy]

  # GET /project_temps
  # GET /project_temps.json
  def index
    @project_temps = ProjectTemp.paginate(:page => params[:page], :per_page => 25)



    # @found = ProjectTemp.search(params['search'], fields: [:title, :country, :sector_name, :client, :region, :owner, :current_phase, :responsible])
    # if @found.length == 0
    #   @found = ProjectTemp.paginate(:page => params[:page], :per_page => 25)
    # end
    #
    @found_sector =  ProjectTemp.search(params['sector_name'], fields: [:sector_name], :per_page => 25)
    if @found_sector.length == 0
      @found_sector = ProjectTemp.paginate(:page => params[:page], :per_page => 25)
    end
  ...
  def create
    ...
    respond_to do |format|
      if @project_temp.save
        format.html { redirect_to [:admin, @project_temp], notice: 'Project temp was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @project_temp.update(project_temp_params)
        format.html { redirect_to [:admin, @project_temp], notice: 'Project temp was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_project_temps_url, notice: 'Project temp was successfully destroyed.' }
