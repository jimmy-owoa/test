class User::BiddingTempsController < User::ApplicationController
  before_action :set_bidding_temp, only: [:show, :edit, :update, :destroy]

  # GET /bidding_temps
  # GET /bidding_temps.json
  def index
    @bidding_temps = BiddingTemp.all
    @found = BiddingTemp.search(params['search'], fields: [:title, :country, :sector, :bidding_company])
    if @found.length == 0
      @found = BiddingTemp.all
    end
    @found = BiddingTemp.search(params[:sector], fields: [:sector]).results &
                    BiddingTemp.search(params[:country], fields: [:country]).results
    if @found.length == 0
      @found = BiddingTemp.all
    end
  ...
  def create
    ...
    respond_to do |format|
      if @bidding_temp.save
        format.html { redirect_to [:user, @bidding_temp], notice: 'Bidding temp was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @bidding_temp.update(bidding_temp_params)
        format.html { redirect_to [:user, @bidding_temp], notice: 'Bidding temp was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to user_bidding_temps_url, notice: 'Bidding temp was successfully destroyed.' }
